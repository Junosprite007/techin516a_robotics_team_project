...
#include "turtlebot3_gix_hardware/turtlebot3_gix_system.hpp"

namespace robotis
{
namespace turtlebot3_gix_hardware
{
auto logger = rclcpp::get_logger("turtlebot3_gix");
hardware_interface::CallbackReturn TurtleBot3GIXSystemHardware::on_init(
  const hardware_interface::HardwareInfo & info)
{
  ...
  gix_acceleration_[0] = stoi(info_.hardware_parameters["dxl_gix_profile_acceleration"]);

  gix_velocity_[0] = stoi(info_.hardware_parameters["dxl_gix_profile_velocity"]);
  ...
  if (opencr_->is_connect_gix()) {
    RCLCPP_INFO(logger, "Connected GIX motor");
  } else {
    RCLCPP_FATAL(logger, "Not connected GIX motor");
    return hardware_interface::CallbackReturn::ERROR;
  }

  dxl_wheel_commands_.resize(2, 0.0);

  dxl_gix_commands_.resize(1, 0.0);
  // dxl_gix_commands_[0] = 0.0;
  dxl_gix_commands_[0] = std::numeric_limits<double>::quiet_NaN();
...
TurtleBot3GIXSystemHardware::export_state_interfaces()
{
...
TurtleBot3GIXSystemHardware::export_command_interfaces()
{
  ...
  command_interfaces.emplace_back(
    hardware_interface::CommandInterface(
      info_.joints[2].name, hardware_interface::HW_IF_POSITION, &dxl_gix_commands_[0]));

  return command_interfaces;
}

hardware_interface::CallbackReturn TurtleBot3GIXSystemHardware::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_INFO(logger, "GIX motor and wheels torque ON");
  opencr_->gix_torque(opencr::ON);
  opencr_->wheels_torque(opencr::ON);

  opencr_->send_heartbeat(heartbeat_++);
  RCLCPP_INFO(logger, "Set profile acceleration and velocity to GIX motor");
  opencr_->set_gix_profile_acceleration(gix_acceleration_);
  opencr_->set_gix_profile_velocity(gix_velocity_);
...
hardware_interface::CallbackReturn TurtleBot3GIXSystemHardware::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::return_type TurtleBot3GIXSystemHardware::read(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  RCLCPP_INFO_ONCE(logger, "Start to read wheels and GIX motor states");
  ...
  dxl_positions_[2] = opencr_->get_gix_positions()[opencr::gix::GIX];
  dxl_velocities_[2] = opencr_->get_gix_velocities()[opencr::gix::GIX];
...
hardware_interface::return_type TurtleBot3GIXSystemHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
...
PLUGINLIB_EXPORT_CLASS(
  robotis::turtlebot3_gix_hardware::TurtleBot3GIXSystemHardware,
